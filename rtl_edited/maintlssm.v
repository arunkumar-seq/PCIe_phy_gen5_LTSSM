...
//Local parameters
    ...
    begin
        ...
        begin
            ...
            // BUGFIX-011/017b: give the formerly latch-inferred outputs
        ...
        begin
            // FSM-001: synchronous re-initialization
        ...
        begin
            ...
            // ------------------------------------------------------------
            // Polling completion capture
            //
            // finishTx/finishRx are one-cycle pulses.
            // Capture them so that TX and RX do not have to finish
            // on the exact same clock cycle.
            // ------------------------------------------------------------
            ...
            if(writeNumberOfDetectedLanes)
                numberOfDetectedLanes<=numberOfDetectedLanesIn;
    
            if(writeLinkNumberTx)
                linkNumber<=linkNumberInTx;
            else if(writeLinkNumberRx)
                linkNumber<=linkNumberInRx;
    
            if(writeUpconfigureCapability)
                upConfigureCapability<=upConfigureCapabilityIn;
    
            if(writeReceiverpresetHintDSP)
                ReceiverpresetHintDSP <=ReceiverpresetHintDSPIn;
    
            if(writeReceiverpresetHintUSP)
                ReceiverpresetHintUSP <=ReceiverpresetHintUSPIn;
    
            if(writeTransmitterPresetHintUSP)
                TransmitterPresetHintUSP<=TransmitterPresetHintUSPIn;
    
            if(writeTransmitterPresetHintDSP)
                TransmitterPresetHintDSP <=TransmitterPresetHintDSPIn;
    
            if(write_directed_speed_change)
                directed_speed_change <= directed_speed_change_In;
    
            if(writeRateId)
                rateId <= rateIdIn;
    
            // BUGFIX-011/017b/017c/017d:
            // Registered updates replacing latch-inferred /
            // multiply-driven combinational outputs.
       ...
       case (currentState)
        ...
        begin
            case ({substateTx,substateRx})
                ...
                begin
                    // TX and RX may complete DetectActive on different cycles.
                    // txPollingDone/rxPollingDone remember an earlier one-cycle
                    // completion pulse until the other side also completes.
                    ...
                    begin
                        {substateTxnext,substateRxnext} =
                            {pollingActive,pollingActive};
                
                        lpifStateStatus = reset_;
                    end
                
                    else if((finishTx && gotoTx == detectQuiet) ||
                            (finishRx && gotoRx == detectQuiet))
                    begin
                        {substateTxnext,substateRxnext} =
                            {detectQuiet,detectQuiet};
